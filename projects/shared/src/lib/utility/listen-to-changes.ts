...
const errorMsg = (key: any): string => `Cannot listen to changes for property ${String(key)} because`;
...
export function listenToChanges<G, K extends keyof G>(
  ...
  function getPropertyDescriptor(_object: any): PropertyDescriptor | undefined {
    if (!_object) {
      return;
    }
